...
def query_h1b_jobs(job_title):
    ...
    return query_db("SELECT * FROM h1b_jobs WHERE `JOB TITLE` LIKE ? LIMIT 5", ('%' + job_title + '%',))

def query_current_jobs(job_title):
    """Query the database for jobs with the specified job title."""
    return query_db("SELECT * FROM current_jobs WHERE `Job-Title` LIKE ? LIMIT 50", ('%' + job_title + '%',))
